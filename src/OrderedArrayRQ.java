...
public class OrderedArrayRQ implements Runqueue {
    ...
    public void enqueue(String procLabel, int vt) {
    	...
    	if (this.procArray.length == 0) {
    		...
    		for (int i = 0; i<this.procArray.length; i++) {
        		...
        		if (newProc.getVT() == this.procArray[i].getVT()) {
        			...
        			equivalent = true;
        			switchCase = 1;
        		//If not, find the index of the first VT in the array which is larger than newProc.VT and break.
    		...
    		case 1:
    			...
    			//There is one or more equivalent VT values in the array - new entry needs to go behind the largest one.
    		...
    		case 2:
    			...
    			//There are no equivalent VT values but the new VT value is not the largest so far.
    			Proc[] newArray1 = new Proc[this.procArray.length + 1];
    			//Add new proc before the element at greaterIndex (first element.VT in the array which is larger than newProc.VT).   			
    			int k = 0;
    			for (int i = 0; i < this.procArray.length; i++) {
    				if (k == greaterIndex) {
    					newArray1[k] = newProc;
    		...
    		case 3:
    			...
    			//The VT value of the new proc is the largest so far. New proc goes at the end of the array.
    	...
    	for (int i = 0; i < this.procArray.length; i++) {
    		System.out.println(this.procArray[i].getProcLabel() + this.procArray[i].getVT());
    	}
    } 
